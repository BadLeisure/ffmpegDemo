...
/**
 * get_clock() 的返回值是 “同步用的物理时间基准”，用于对齐音视频流，不适合直接反推媒体时间
 * 进度条需要的是 “当前媒体时间”，应直接使用 ffplay 中记录的媒体时间变量（如 audio_clock），而非通过 get_clock() 计算。
   核心原因：媒体时间是 “内容进度的绝对坐标”，与播放速度无关；而 get_clock() 是 “同步用的相对基准”，会随速度变化动态调整，无法直接反映内容进度
 */ 
...
/**
 * set_clock_at 传入的 pts 是经过速率转换后的实际物理时间（用户真实感知的播放进度），
 * get_clock 最终返回的也是实时的实际物理时间。这两个函数配合，确保时钟始终反映 “用户当前实际听到 / 看到的播放位置”，而非原始媒体时间。
 */
