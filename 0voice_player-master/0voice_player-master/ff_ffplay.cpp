...
static int audio_decode_frame(FFPlayer *is)
{
    ...
    if (!std::isnan(af->pts)) {
        //af->frame->nb_samples:当前音频帧包含的样本数  af->frame->sample_rate：音频的采样率（单位：样本 / 秒)
...
//实时统计音视频队列的缓存数据量（bytes）、数据包数量（packets）和总时长（duration）
...
int FFPlayer::stream_has_enough_packets(AVStream * st, int stream_id, PacketQueue * queue)
{
    //流 ID 无效（如未找到对应流）
    return stream_id < 0 ||
            //队列已收到终止请求（如用户停止播放），此时无需继续缓存数据
           queue->abort_request ||
           (st->disposition & AV_DISPOSITION_ATTACHED_PIC) ||
           //队列中缓存的数据包数量超过最小帧数（MIN_FRAMES，如 25 帧）,队列中数据的总时长（转换为秒）超过 1 秒，或队列未记录时长（queue->duration 为 0，此时忽略时长判断）。
...
int FFPlayer::read_thread()
{
    ...
    while (1) {
        ...
        if(ret < 0) { // 出错或者已经读取完毕了
            ...
            /**
             * 对实时流，预留 “重试机会” 避免误判结束；
               对解码器，预留处理空包的时间，保证播放正常收尾；
               对 IO 异常，通过重试应对临时错误，提升稳定性
             */
...
/**
 * REFRESH_RATE（10ms）的作用：这是线程的 “最小检查间隔”。即使当前帧不需要立即显示，线程也会每 10ms 唤醒一次
 * 检查是否有新的同步指令（如用户拖动进度条、音频时钟变化等），保证同步的实时性
 */
...
retry:
        if (frame_queue_nb_remaining(&pictq) == 0) {
            ...
            //若当前帧的序列编号与视频队列不一致（如seek后序列更新），则丢弃该帧并重试
            if (vp->serial != videoq.serial) {
                frame_queue_next(&pictq);
                goto retry;
            }
            if (lastvp->serial != vp->serial) {
                //重置 frame_timer 是为了在播放序列切换时（如 seek、首次播放），将帧的显示基准时间同步到当前系统时间
            ...
            //判断是否到显示时间，若未到则计算等待时间,FFMIN 确保等待时间不超过线程的基础刷新间隔（避免过度休眠
            ...
            //frame_timer += delay;  // 上一帧显示时间 + 延迟 → 当前帧的理论显示时间
            frame_timer += delay;
            //若当前系统时间比 frame_timer 晚太多（如 CPU 繁忙导致帧显示延迟），重置 frame_timer 为当前时间，避免后续帧连续延迟（防止画面 “跳变”）
            ...
            if (!std::isnan(vp->pts)) {
            //这个是更新待显示的视频帧的时间戳
            //视频在同步时也会动态设置自己的基准，逻辑和音频完全一致 —— 即根据当前帧的媒体时间和播放速率，实时计算 “等效物理时间” 作为视频时钟的基准
             ...
             * // 丢帧条件：
               // 1. 不是单步播放（step为false）；
               // 2. 允许丢帧（framedrop>0），且主时钟不是视频（避免视频自己同步自己时丢帧）；
               // 3. 当前时间已超过当前帧的显示结束时间（frame_timer + duration）→ 帧已过时
             */
    ...
    for (;;) {
        ...
        if (queue_->serial == pkt_serial_) { // 1.1 先判断是否是同一播放序列的数据
            do {
                    ...
                    case AVMEDIA_TYPE_AUDIO:
                        ...
                        if (ret >= 0) {
                            ...
                            if (frame->pts != AV_NOPTS_VALUE) {
                                ...
                                //pts 在代码中已被转换为 “基于采样率的时间基”（tb = {1, sample_rate}）
                ...
                //只有当代码发送（空数据包）后，解码器会启动 “结束流程”：处理剩余数据→输出所有帧→最终返回AVERROR_EOF，在这个代码里面是packet_queue_put_nullpacket放入的packet
                if (ret == AVERROR_EOF) {
        ...
        if (pkt.data == flush_pkt.data) {//
            ...
            if (avctx_->codec_type == AVMEDIA_TYPE_SUBTITLE) {
                ...
                //用于标记 “上次发送数据包时因解码器忙碌而暂存的包”（如 avcodec_send_packet 返回 AVERROR(EAGAIN) 时，会将包暂存为 pkt_）
...
int Decoder::audio_thread(void *arg)
{
    ...
    do {
        ...
        if (got_frame) {
            ...
            // 3. 设置Frame并放入FrameQueue,从采样率转换成秒
