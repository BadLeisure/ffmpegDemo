...
//内部包含一系列键值对（如 width、height、framerate 等），parseScriptTag 会从中提取这些信息并保存到全局变量
...
//            LogInfo("RTMPPacket_m_packetType:%d\n", packet.m_packetType);
            if (packet.m_packetType == RTMP_PACKET_TYPE_VIDEO)
            {
                ...
                //通常在视频流开始时、分辨率变化时或周期性发送（确保解码器重新初始化时能获取配置）
                //详细见rtmp拉流笔记截图
...
//                            packet.m_nBodySize);
                ...
                if (keyframe && sequence)
                {
                 ...
                 //1（视频信息）+ 1（AVCPacketType） + 3（CompositionTime） = 5 字节  + AVCDecoderConfigurationRecord ，前面 5 个固定字段（configurationVersion 到 lengthSizeMinusOne）共占 5 字节 = 10 字节
                    uint32_t offset = 10;
                    //sps pps最多31位
                    ...
                    //av_packet_from_data是 FFmpeg 的函数，用于将已分配的内存缓冲区与AVPacket绑定，并做基础校验（如数据指针非空、大小合理）
                    if(av_packet_from_data(&sps_pkt, sps_pkt.data, sps_pkt.size) == 0) {
                        //校验通过后，用memcpy将之前解析并缓存的 SPS 数据（带起始码0x00000001）复制到AVPacket的数据区，完成封装
                        ...
                        //fflush(rtmp_dump_h264); // 立即刷新缓冲区，确保数据写入文件
                ...
                else
                {
                    ...
                    //1（视频信息）+ 1（AVCPacketType） + 3（CompositionTime） = 5 字节  
                    uint32_t offset = 5;
                    total_slice_size = 0;
...
//                        LogInfo("nalu_type:%d", nalu_type);
                        //第一个 NALU 用 4 字节起始码（0x00000001），后续用 3 字节（0x000001）” 的设计，是兼顾解码兼容性和数据传输效率的行业惯例
                        if(first_slice) {
                            first_slice = 0;
                        ...
                        //offset += data_len：跳过当前分片的数据，定位到下一个分片的长度字段
                        offset += data_len; 
                        //total_slice_offset是当前 RTMP 包内所有重组后 NALU 数据的总长度（可能包含 1 个或多个完整 NALU）
...
//                        sample_frequency_index_ = 5;
            ...
            {
                /**
                 * 这些命令通常对应 RTMP 协议中的 “命令消息（Command Message）” 或 “控制消息（Control Message）”，例如：
                    播放相关：play（开始播放）、pause（暂停播放）、stop（停止播放）。
                    定位相关：seek（跳转播放位置）、onSeek（服务器反馈跳转结果）。
                    会话控制：connect（建立连接）、createStream（创建流）、deleteStream（删除流）。
                    状态反馈：服务器返回的状态信息（如播放开始、缓冲状态、错误提示等）。
                 * 
                 * 
                 */
                LogInfo("can't handle it ");
                /**
                 * 循环从网络读取服务器发送的 RTMP 数据包（如命令包、数据包等）
                 * 根据数据包的类型（例如是 invoke 命令、音频 / 视频数据等），调用对应的处理函数（例如处理服务器指令的 HandleInvoke，处理数据的 RTMP_ClientPacket 等）
                 * 简单说，RTMP_Client 是 “流程控制器”，它本身不直接判断拉流还是推流，而是通过调用子函数（如 HandleInvoke）来完成具体逻辑
                 */
